...
using Manga_Rica_P1.UI.Reportes.Shared;     // FechaFiltroSidebar
...
{
    public partial class FormReporteHorasDiarias : Form
    {
        ...
        private string _titulo = "Horas_Diarias";

        public FormReporteHorasDiarias(string connectionString, string virtualHost = "appassets")
        {
            InitializeComponent();
            ...
            // ===== Configurar sidebar de fecha =====
            filtroFecha.Titulo = "Filtro de Fecha";
            filtroFecha.EtiquetaCampo = "Fecha :";
            filtroFecha.TextoBoton = "Generar reporte";

            filtroFecha.BuscarPorFecha += FiltroFecha_BuscarPorFecha;

            // Inicializar WebView2 al cargar el form
            Load += FormReporteHorasDiarias_Load;
        }

        // Inicializar WebView2 sin cargar reporte aún
        private async void FormReporteHorasDiarias_Load(object? sender, EventArgs e)
        {
            try
            {
...
<body>
  <h3>Horas Diarias</h3>
  <p>Seleccione una <b>fecha</b> en el panel izquierdo y presione <b>Generar reporte</b>.</p>
</body>
</html>";
        ...
        private async void FiltroFecha_BuscarPorFecha(
            object? sender,
            FechaFiltroSidebar.BuscarPorFechaEventArgs e)
        {
            await CargarReporteAsync(e.Fecha);
        }

        private async Task CargarReporteAsync(DateTime fecha)
        {
            try
            {
                var vm = await _service.GenerarReporteAsync(fecha);
                ...
                var html = await _renderer.RenderAsync(
                    templateName: "HorasDiarias.cshtml",
                    model: vm,
                    title: vm.Titulo,
                ...
                _vm = vm;
                _titulo = $"Horas_Diarias_{vm.Fecha:yyyyMMdd}";
            }
            catch (Exception ex)
            {
                MessageBox.Show(
                    $"Error al generar el reporte de horas diarias: {ex.Message}",
                    "Horas Diarias",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
            }
        ...
        private async void btnExportExcel_Click(object? sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(
                    "Primero genere el reporte para alguna fecha.",
                    "Exportar Excel",
                    MessageBoxButtons.OK,
            ...
            var title = _titulo ?? "Horas Diarias";
            ...
            await ReportExportHelper.ExportStyledXlsxOrCsvAsync(
                ...
                groupByColumn: null,                         // sin agrupación
                subtotalColumns: Array.Empty<string>(),      // sin subtotales por grupo
                moneyColumns: Array.Empty<string>(),         // no son montos en dinero
