...
namespace Manga_Rica_P1.UI.Solicitudes.Modales
{
    public class AddSolicitud : Form
    {
        ...
        public AddSolicitud(EntitySolicitud seed = null)
        {
            ...
            // Fila 4 (Teléfono izquierda, Celular derecha)
            var lblTelefono = new Label { Text = "Teléfono", AutoSize = true, Left = L, Top = 80 + 3 * (H + G) };
            txtTelefono.SetBounds(L, 100 + 3 * (H + G), W, H);

            var lblCelular = new Label { Text = "Celular", AutoSize = true, Left = R, Top = 80 + 3 * (H + G) };
            txtCelular.SetBounds(R, 100 + 3 * (H + G), W, H);

            // Fila 5 (Nacionalidad)  ⬅️ MOVIDA 20px MÁS ABAJO PARA EVITAR SOLAPE
            var lblNacionalidad = new Label { Text = "Nacionalidad", AutoSize = true, Left = L, Top = 100 + 4 * (H + G) };
            txtNacionalidad.SetBounds(L, 120 + 4 * (H + G), W, H);

            // Fila 6 (¿Laboró?)
            chkLaboro.Text = "¿Laboró anteriormente?";
            chkLaboro.SetBounds(L, 120 + 5 * (H + G), 220, H);

            // Fila 7 (Dirección multilínea)
            var lblDireccion = new Label { Text = "Dirección", AutoSize = true, Left = L, Top = 160 + 5 * (H + G) };
            txtDireccion.SetBounds(L, 180 + 5 * (H + G), 600, 100);
            txtDireccion.Multiline = true;

            // ===== Layout botones =====
            int buttonsTop = txtDireccion.Bottom + 12;   // botones justo debajo de Dirección
            ...
            if (seed != null)
            {
                ...
                cboEstadoCivil.SelectedItem =
                    cboEstadoCivil.Items.Cast<object>().FirstOrDefault(x => x.ToString() == seed.Estado_Civil)
                    ?? cboEstadoCivil.Items.Cast<object>().FirstOrDefault();
                txtTelefono.Text = seed.Telefono ?? string.Empty;
        ...
        private void OnSave()
        {
            ...
            // Mapear a entidad (Teléfono opcional)
            ...
            Result.Telefono = string.IsNullOrWhiteSpace(txtTelefono.Text) ? null : txtTelefono.Text.Trim();
